...
def parse(self, exclusive=False):
    ...
    for ann in self.FLAGS.annotation:
        if not os.path.isdir(ann):
            msg = 'Annotation directory not found {} .'
            exit('Error: {}'.format(msg.format(ann)))
    print('\n{} parsing annotations for {} sequences'.format(meta['model'], len(self.FLAGS.annotation)))
    dumps = pascal_voc_clean_xml(self.FLAGS.annotation, self.FLAGS.dataset, meta['labels'], exclusive)
    return dumps
...
def _batch(self, chunk):
    ...
    path = os.path.join(self.FLAGS.dataset[seq_idx], jpg)
    if not os.path.exists(path):
        raise IOError('Image {} does not exist'.format(path))

    # if not os.path.exists(path):
    #     fixed_jpg = 'image{:06d}.jpg'.format(int(jpg.split('image')[1].split('.')[0]))
    #     fixed_path = os.path.join(self.FLAGS.dataset[seq_idx], fixed_jpg)
    #     if not os.path.exists(fixed_path):
    #         raise IOError('Neither {} nor {} exist'.format(path, fixed_path))
    #     path = fixed_path
    #     jpg = fixed_jpg
