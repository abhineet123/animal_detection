...
try:
    ...
    from siamfc.src.convolutional import set_convolutional
    from siamfc.src.crops import extract_crops_z, extract_crops_x, pad_frame, resize_images
...
def build_tracking_graph2(final_score_sz, design, env):


    _pos_x_ph = tf.placeholder(tf.float64)
    _pos_y_ph = tf.placeholder(tf.float64)
    _z_sz_ph = tf.placeholder(tf.float64)
    _x_sz0_ph = tf.placeholder(tf.float64)
    _x_sz1_ph = tf.placeholder(tf.float64)
    _x_sz2_ph = tf.placeholder(tf.float64)
    ...
    image = tf.placeholder(tf.float32, shape=None, name='image')
    ...
    if design.pad_with_image_mean:
        ...
        avg_chan = None
    # pad with if necessary
    frame_padded_z, npad_z = pad_frame(image, frame_sz, _pos_x_ph, _pos_y_ph, _z_sz_ph, avg_chan)
    frame_padded_z = tf.cast(frame_padded_z, tf.float32)
    # extract tensor of z_crops
    z_crops = extract_crops_z(frame_padded_z, npad_z, _pos_x_ph, _pos_y_ph, _z_sz_ph, design.exemplar_sz)
    frame_padded_x, npad_x = pad_frame(image, frame_sz, _pos_x_ph, _pos_y_ph, _x_sz2_ph, avg_chan)
    frame_padded_x = tf.cast(frame_padded_x, tf.float32)
    # extract tensor of x_crops (3 scales)
    x_crops = extract_crops_x(frame_padded_x, npad_x, _pos_x_ph, _pos_y_ph, _x_sz0_ph, _x_sz1_ph, _x_sz2_ph, design.search_sz)
    # use crops as input of (MatConvnet imported) pre-trained fully-convolutional Siamese net
    template_z, templates_x, p_names_list, p_val_list = _create_siamese(os.path.join(env.root_pretrained,design.net), x_crops, z_crops)
    template_z = tf.squeeze(template_z)
    templates_z = tf.stack([template_z, template_z, template_z])
    ...
    pos = [_pos_x_ph, _pos_y_ph]
    sizes = [_z_sz_ph, _x_sz0_ph, _x_sz1_ph, _x_sz2_ph]
    return image, templates_z, scores_up, pos, sizes
