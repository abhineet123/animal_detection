...
class Params:
    """
    :ivar aug_seq_prefix:
    :type aug_seq_prefix: str

    :ivar aug_seq_size:
    :type aug_seq_size: int

    :ivar batch_size:
    :type batch_size: int

    :ivar bkg_bboxes_csv:
    :type bkg_bboxes_csv: str

    :ivar bkg_iou_thresh:
    :type bkg_iou_thresh: float
    ...
    :ivar border_ratio:
    :type border_ratio: float

    :ivar flip_lr_prob:
    :type flip_lr_prob: float

    :ivar hist_match_type:
    :type hist_match_type: int
    ...
    :ivar inclue_src_in_augmented:
    :type inclue_src_in_augmented: int
    ...
    :ivar make_square:
    :type make_square: int

    :ivar mask_border:
    :type mask_border: int
    ...
    :ivar mask_postfix:
    :type mask_postfix: str

    :ivar mask_root_dir:
    :type mask_root_dir: str

    :ivar mask_threshold:
    :type mask_threshold: int

    :ivar mask_type:
    0: no mask
    1: get2DGaussianErrorFunctionArray
    2: get2DGaussianArray
    3: get2DGaussianArray2
    4: read segmentation mask from image file
    :type mask_type: int
    ...
    :ivar only_one_src_obj:
    :type only_one_src_obj: int

    :ivar random_bkgs:
    :type random_bkgs: int

    :ivar sample_frg_per_bkg: no. of foreground images per background image, i.e. number of synthetic images
    generated out of each background image, one each from each foreground image
    :type sample_frg_per_bkg: int
    ...
    :ivar src_img_per_seq:
    :type src_img_per_seq: int

    :ivar src_paths:
    :type src_paths: str

    :ivar src_postfix:
    :type src_postfix: str
    ...
    def __init__(self):
        self.cfg = ()
        self.aug_seq_prefix = ''
        self.aug_seq_size = 1000
        self.batch_size = 1
        self.bkg_bboxes_csv = ''
        self.bkg_iou_thresh = 0.1
        self.bkg_path = ''
        self.bkg_size = '1280x720'
        self.border_ratio = 0.0
        self.flip_lr_prob = 0.0
        self.hist_match_type = 0
        self.img_dir = ''
        self.img_ext = 'jpg'
        self.inclue_src_in_augmented = 1
        self.labels_path = 'data/wildlife_label_map.pbtxt'
        self.load_bkg = 1
        self.load_path = ''
        self.make_square = 1
        self.mask_border = 0
        ...
        self.n_classes = 7
        self.n_frames = 0
        self.only_one_src_obj = 0
        self.random_bkgs = 0
        self.sample_frg_per_bkg = 0
        self.save_path = ''
        self.show_img = 1
        self.src_img_per_seq = 0
        self.src_paths = ''
        self.src_postfix = ''
        self.src_root_dir = ''
        self.vis_size = '1280x720'
        self.visualize = 0
...
def main():
    ...
    """read background boxes for suitable locations where to add the new object"""
    ...
    """dict of dict --> all boxes for each file grouped by class"""
    bkg_data_dict = {}
    for _, row in df.iterrows():
        ...
        if filename not in bkg_data_dict:
            bkg_data_dict[filename] = {}
        if class_name not in bkg_data_dict[filename]:
            bkg_data_dict[filename][class_name] = []
        bkg_data_dict[filename][class_name].append(bbox)

    """get all source image sequence paths"""
    if src_paths:
    ...
    if mask_paths or mask_root_dir:
        """masks need to be read from image files"""
        mask_type = 4

    """get all mask image paths corresponding to the source images"""
    if mask_type == 4:
        if mask_paths:
            ...
            raise AssertionError('Either mask_paths or mask_root_dir must be provided for loading external masks')

        assert len(mask_paths) == len(src_paths), 'Mismatch between no. of mask_paths: {} and src_paths: {}'.format(
    ...
    """Load or read background images"""
    if bkg_size:
    ...
    """optionally read background boxes from an external CSV file in case these boxes are not part of the 
    annotations of the background sequence which had earlier been read into the dictionary"""

    """single level dictionary unlike the earlier one and class agnostic"""
    external_bkg_bboxes = {}
    ...
    if sample_frg_per_bkg > 0:
        """collect paths of all images over all source/foreground sequences for sampling"""
        for seq_id in range(n_seq):
            ...
            assert total_frames > 0, 'No input frames found for seq {}'.format(src_path)
        ...
        assert sample_frg_per_bkg <= n_frgs, 'sample_frg_per_bkg: {} exceeds n_frgs: {}'.format(
            sample_frg_per_bkg, n_frgs)

        n_samples = n_bkgs * sample_frg_per_bkg  # no. of foreground images actually needed
            ...
            while _aug_id < _n_aug:
                ...
                if external_bkg_bboxes:
                    ...
                    if len(matching_fnames) > 1:
                        raise IOError(
                            'Multiple matching filenames found for {}:\n {}'.format(out_prefix, matching_fnames))
                ...
                for obj_id in range(n_bkg_boxes):
                    ...
                    if flip_lr_prob == -1:
                        ...
                        if external_bkg_bboxes_img is None:
                            raise IOError(
                                'external_bkg_bboxes_file could not be read: {}'.format(external_bkg_bboxes_file))
                        ...
                        external_bkg_bboxes_patch = cv2.resize(external_bkg_bboxes_patch,
                                                               orig_src_patch.shape[:2][::-1])
                ...
                if seq_frame_id == aug_seq_size and frame_id > 0:
                    ...
                    #     raise IOError('Amazingly annoying mismatch between len(csv_raw): {} and len(filenames): {
                    #     }'.format(
