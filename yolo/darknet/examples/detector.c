...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        printf("Step %ld: %f, %f avg, %f rate, %lf seconds, %d images\n", 
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        if(filename){
            strncpy(input, filename, 512);
        } else {
            printf("Enter Image Path: ");
            fflush(stdout);
            input = fgets(input, 512, stdin);
            if(!input) return;
            strtok(input, "\n");
        }
		printf("input: %s\n", input);
        ...
        time=what_time_is_it_now();
        network_predict(net, X);
		double det_time = what_time_is_it_now() - time;
        printf("%s: Predicted in %f seconds (%f fps).\n", input, det_time, 1.0/ det_time);
        int nboxes = 0;
		...
		const char* input_ext = get_filename_ext(input);
		int extlen = strlen(input_ext), input_len = strlen(input);
		char buff_txt[512], buff_txt2[512];
		char *det_path = buff_txt, *det_path2 = buff_txt2;
		const char* det_ext = "txt";
		//strncpy(det_path, input, input_len - extlen);
		sprintf(det_path, "%s.%s", input, det_ext);
		...
		write_detections(im, dets, nboxes, thresh, names, alphabet, l.classes, det_path);
        free_detections(dets, nboxes);
        if(outfile){
